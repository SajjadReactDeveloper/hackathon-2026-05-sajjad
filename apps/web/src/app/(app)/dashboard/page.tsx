...
export default async function DashboardPage() {
  ...
  try {
    ...
    if (workspace) {
      ...
      tiles = t; revenueData = r; messagesData = m; heatmapData = h;
    }
  } catch { /* show zeros */ }

  const tileValues: Record<string, string> = {
    todayRevenue:      formatPKR(tiles.todayRevenueCents),
    monthRevenue:      formatPKR(tiles.monthRevenueCents),
    openConversations: tiles.openConversations.toString(),
    totalOrders:       tiles.totalOrders.toString(),
    totalMessages:     tiles.totalMessages.toLocaleString(),
    aiReplyRate:       `${tiles.aiReplyRate}%`,
  };
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="px-7 pt-7 pb-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">{workspaceName} · Performance overview</p>
          </div>
      ...
      <div className="px-7 pb-7 space-y-5">
        {/* KPI tiles — full gradient */}
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
          {TILE_CONFIGS.map(({ key, label, sub, href, gradient, shadow, icon }) => (
            <Link
              key={key}
              href={href}
              className="relative overflow-hidden rounded-2xl p-5 text-white block transition-transform hover:-translate-y-0.5 hover:shadow-xl"
              style={{
                background: gradient,
        ...
        >
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #ec4899, #db2777)', boxShadow: '0 4px 14px rgba(236,72,153,0.3)' }}
          >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-semibold text-slate-900 leading-none">AI Engine — Groq Llama 3.3 70B</p>
            <p className="text-[11px] text-slate-400 mt-1">
              Language-mirrored replies (Urdu / English / Roman Urdu) · 3-layer memory · Tool calling · Voice + Image orders
            </p>
          </div>
          <div className="flex items-center gap-4 shrink-0">
            <div className="text-center">
              ...
              <span
                className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full mt-1"
                style={{ background: 'rgba(34,197,94,0.1)', color: '#16a34a' }}
              >
                <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                Auto-Reply ON
            ...
            <div className="text-center">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Tools</p>
              <p className="text-[13px] font-bold text-slate-900 mt-1">8 active</p>
            </div>
            <div className="text-center">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Memory</p>
              <p className="text-[13px] font-bold text-slate-900 mt-1">3 layers</p>
            </div>
            <Link
              href="/settings/ai"
              className="text-[12px] font-semibold px-4 py-2 rounded-xl text-white transition-all"
